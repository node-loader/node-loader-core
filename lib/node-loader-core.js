...
export const resolve = createHook(
  "resolve",
  (specifier, context, defaultResolve) => {
    // ignore node loader configs to avoid inf loop in importing the node loader config
    if (specifier === configPath) {
...
function createHook(name, specialHandler) {
  return async function (...args) {
    const specialResult = specialHandler && specialHandler(...args);
    if (specialResult) {
